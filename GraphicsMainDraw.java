...
public class GraphicsMainDraw extends JFrame {
	...
	private static int cloudnextX, cloudnextY;
	private static int cacnextX, cacnextY, cacccounter, cactype;
	...
	class cactus {
		...
		public cactus(int x, int y, int sizeX, int sizeY, Image cactusImg) {
			...
			// 생성위치
			cacccounter = (int) ((Math.random() * 25 + this.sizeX / 2 + 10) * (2 - accel));
			cactype = (int) (Math.random() * 7) + 0;
		}
	...
	class cloud {
		...
		public cloud(int x, int y, Image cloudImg) {
			...
			cloudList.add(this);
			cloudnextX = (int) (Math.random() * 200 + 1000);
			cloudnextY = (int) (Math.random() * 81 + 20);
		}
	...
	public void GenerateProcess() {
		...
		if (cloudList.get(cloudList.size() - 1).getX() < 700) {
			new cloud(cloudnextX, cloudnextY, cloudImg);
		}
		...
		if (cacccounter <= 0) {
			...
			default:
				throw new IllegalArgumentException("Unexpected value: " + cactype);
			}
